...
#AA = [A*i for i in np.linspace(-2,2,17)]
#BB = [B*i for i in np.linspace(-2,2,17)]
#CC = np.linspace(-15,15,17)
#KK = np.linspace(-15,15,17)
#oocc_checked = []
#for i in range(4):
#    if i == 0:
#        oocc_checked.extend([[ainA,B,0.,0.] for ainA in AA])
#    if i == 1:
#        oocc_checked.extend([[A,binB,0.,0.] for binB in BB])
#    if i == 2:
#        oocc_checked.extend([[A,B,cinC,0.] for cinC in CC])
#    if i == 3:
#        oocc_checked.extend([[A,B,0.,kinK] for kinK in KK])

#evols_r = varOort(oocc_checked)
#np.save('/fs/nas14/other0/cbenitez/evolsOC/evols_oc_var1',evols_r)
#np.save('/fs/nas14/other0/cbenitez/evolsOC/evols_oc_var1_cons',oocc_checked)

AA = [A*i for i in np.linspace(-2,2,9)]
BB = [B*i for i in np.linspace(-2,2,9)]
CC = np.linspace(-11.5,11.5,9)
KK = np.linspace(-11.5,11.5,9)
oocc_checked = []
for i in range(6):
    if i == 0:
...
evols_r = varOort(oocc_checked)
np.save('/fs/nas14/other0/cbenitez/evolsOC/evols_oc_var2_go',evols_r)
np.save('/fs/nas14/other0/cbenitez/evolsOC/evols_oc_var2_cons_go',oocc_checked)
