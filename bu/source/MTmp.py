...
tardir = "/fs/nas14/other0/cbenitez/partsSim"
...
class rop:
    ...
    def __init__(self,limits=None,op=None):
        ...
        self.dens_unit = output.info["unit_density"].express(C.H_cc)/1.31533157895*1.00784
...
#####################################################################
# Runge-Kutta 4
#Aguas, cambie h a ser global para meterlo a multiprocessing. Tambien force_T,
#entonces los kargs time_step y fuerza estan de mas. Los dejo por "razones historicas"
#las unidades son kpc y kpcs^{-1}
#####################################################################
h=0
def RK4(xxvv,time_step=h,fuerza=force_T):

    k_1 = np.array(map(lambda xv: np.append(xv[3::],force_T(xv[0],xv[1],xv[2])),xxvv))
    xxvv_1 = xxvv+k_1*h/2

    k_2 = np.array(map(lambda xv: np.append(xv[3::],force_T(xv[0],xv[1],xv[2])),xxvv_1))
    xxvv_2 = xxvv+k_2*h/2

    k_3 = np.array(map(lambda xv: np.append(xv[3::],force_T(xv[0],xv[1],xv[2])),xxvv_2))
    xxvv_3 = xxvv+k_3*h

    k_4 = np.array(map(lambda xv: np.append(xv[3::],force_T(xv[0],xv[1],xv[2])),xxvv_3))
...
def evolution(op_0,op_f,limits, NT, rv_LSR, rho_lim, box_dims, path_file, name_file):
    ...
    where = np.array([np.where(abs(rv_LSR[0]-sf.xx)<box_w[0])[0],np.where(abs(rv_LSR[1]-sf.yy)<box_w[1])[0],np.where(abs(rv_LSR[2]-sf.zz)<box_w[2])[0]])
    ...
    xxvv_RK4.extend([np.array([xx_picos[i],yy_picos[i],zz_picos[i],vvx_picos[i],vvy_picos[i],vvz_picos[i]]) for i in range(len(where_picos[0]))])
    ...
    with open(path_file+name_file_LSR,"a") as cfile:
            cfile.write(
                        str(rv_LSR[0])+' '+str(rv_LSR[1])+' '+str(rv_LSR[2])+' '+ \
                        str(rv_LSR[3])+' '+str(rv_LSR[4])+' '+str(rv_LSR[5])+' '+ \
                        str(creation_time)+' '+str(op_0)+'\n')


#    for i in range(len(xxvv_RK4)):
#
#        with open(path_file+name_file,"a") as cfile:
#            cfile.write(
#                        str(xxvv_RK4[i][0])+' '+str(xxvv_RK4[i][1])+' '+str(xxvv_RK4[i][2])+' '+ \
#                        str(xxvv_RK4[i][3])+' '+str(xxvv_RK4[i][4])+' '+str(xxvv_RK4[i][5])+' '+ \
    ...
    while op_now<=op_f:
        ...
        with contextlib.closing(Pool()) as pool:
            for t in range(NT):
                ...
                rv_LSR = RK4([rv_LSR])[0]
                xxvv_RK4 = list(pool.map(RK4,[xxvv_RK4])[0])
...
#                            str(dd_RK4[i])+' '+str(creation_times[i])+' '+str(op_now)+' '+str(ids[i+1])+'\n')
    ...
    with open(path_file+'info_'+name_file,"a") as cfile:
        cfile.write("op_0 = "+str(op_0)+" \n" + \
                    ...
                    "NT = "+str(NT)+" \n" +\
                    "r_LSR = ("+str(rv_LSR[0])+","+str(rv_LSR[1])+","+str(rv_LSR[2])+") \n"+\
                    "v_LSR = ("+str(rv_LSR[3])+","+str(rv_LSR[4])+","+str(rv_LSR[5])+") \n"+ \
                    "rho_lim = "+str(rho_lim)+" \n"+ \
